...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        element.innerHTML = `
            <img src="${games[i].img}" width="300px" style="border-radius: 10px"/>
            <h4>${games[i].name}</h4>
...
const contributors = GAMES_JSON.reduce((acc, game) => acc + game.backers , 0);
...
const totalAmountRaised = GAMES_JSON.reduce((acc, card) => card.pledged + acc, 0);
...
const unfundedGames = GAMES_JSON.filter((game) => game.pledged < game.goal).reduce((acc, game) => acc + 1, 0); 

// create a string that explains the number of unfunded games using the ternary operator
const raisedGames = GAMES_JSON.filter((game) => game.pledged > 0).length;
const displayStr = `
    A total of 
    $${totalAmountRaised.toLocaleString('en-US')} 
    has been raised for 
    ${raisedGames} 
    ${raisedGames > 1 ? 'games' : 'game'} 
    .Currently, 
    ${unfundedGames} 
...
const addedStr = document.createElement('p');
addedStr.innerHTML = displayStr;
descriptionContainer.appendChild(addedStr);
...
secondGameContainer.appendChild(runnerUpGame);s